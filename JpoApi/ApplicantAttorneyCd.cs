...
namespace JpoApi
{
    ...
    public class ApplicantAttorneyCd : IDisposable
    {
        ...
        public ApplicantAttorneyCd(string applicantAttorneyCd,string a_access_token, string cacheDir = null)
        {
            try
            {
                m_error = e_NONE;
                ...
                {
                    m_cacheDir = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\ayaki\jpoapi\applicant_attorney_cd";
                    if (Directory.Exists(m_cacheDir) == false)
                    {
                        Directory.CreateDirectory(m_cacheDir);
                    }
                }
                m_jsonFile = m_cacheDir + @"\" + applicantAttorneyCd + ".json";
            ...
            read(applicantAttorneyCd, a_access_token);
        }
        // キャッシュの存在チェック
        private int isCache()
        {
            if (File.Exists(m_jsonFile))
            {
                ...
                if (fi.LastWriteTime.Day <= dt.AddDays(ac.m_cacheEffective).Day)
                {
                    using (JpoHttp jpo = new JpoHttp())
                    {
                        jpo.m_json = File.ReadAllText(m_jsonFile);
                        CApplicantAttorneyCd applicant = JsonConvert.DeserializeObject<CApplicantAttorneyCd>(jpo.m_json);
        ...
        private void read(string applicantAttorneyCd, string a_access_token)
        {
            try
            {
                using (JpoHttp jpo = new JpoHttp())
                {
                    ...
                    jpo.get(Properties.Settings.Default.url + @"/applicant_attorney_cd/" + applicantAttorneyCd, a_access_token);
                    while (DateTime.Now.Second % 6 == networkState.m_i64macaddress % 6)
                    {
                        System.Threading.Thread.Sleep(100);
                    }
                    if (jpo.m_error == e_NONE)
                    {
                        CApplicantAttorneyCd applicant = JsonConvert.DeserializeObject<CApplicantAttorneyCd>(jpo.m_json);
