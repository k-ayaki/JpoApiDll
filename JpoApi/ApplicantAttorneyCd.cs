...
namespace JpoApi
{
    ...
    public class ApplicantAttorneyCd : IDisposable
    {
        ...
        public ApplicantAttorneyCd(string applicantAttorneyCd,string accessToken, string cacheDir = null)
        {
            m_error = e_NONE;
            ...
            try
            {
                if (cacheDir == null)
                {
                    cacheDir = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\ayaki\jpoapi\applicant_attorney_cd";
                    Directory.CreateDirectory(cacheDir);
                }
                m_jsonFile = cacheDir + @"\" + applicantAttorneyCd + ".json";
            ...
            try {
                ...
                jpo.get(Properties.Settings.Default.url + @"/applicant_attorney_cd/" + applicantAttorneyCd, accessToken);
                while (DateTime.Now.Second % 6 == networkState.m_i64macaddress % 6)
                {
                    System.Threading.Thread.Sleep(100);
                }
