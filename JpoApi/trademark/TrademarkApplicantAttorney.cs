...
namespace JpoApi
{
    public class TrademarkApplicantAttorney : IDisposable
    {
        ...
        private class CTrademarkApplicantAttorney
        {
            public CResult result { get; set; }
        }
        public string m_responseFile { get; set; }
        public string m_response { get; set; }
        public TrademarkApplicantAttorney(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/trademark/v1/applicant_attorney/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CTrademarkApplicantAttorney jsonObj = JsonConvert.DeserializeObject<CTrademarkApplicantAttorney>(m_response);
