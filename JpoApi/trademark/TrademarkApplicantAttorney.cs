    ...
    public class TrademarkApplicantAttorney : IDisposable
    {
        ...
        public TrademarkApplicantAttorney(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_json = jsonCache.GetJson("api/trademark/v1/applicant_attorney/" + applicationNumber);
                    this.m_jsonFile = jsonCache.m_jsonFilePath;
                    this.m_error = jsonCache.m_error;

                    if (m_json.Length > 0)
                    {
                        CTrademarkApplicantAttorney jsonObj = JsonConvert.DeserializeObject<CTrademarkApplicantAttorney>(m_json);
