    ...
    public class TrademarkRegistrationInfo : IDisposable
    {
        ...
        public TrademarkRegistrationInfo(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_json = jsonCache.GetJson("api/trademark/v1/registration_info/" + applicationNumber);
                    this.m_jsonFile = jsonCache.m_jsonFilePath;
                    this.m_error = jsonCache.m_error;

                    if (m_json.Length > 0)
                    {
                        CTrademarkRegistrationInfo jsonObj = JsonConvert.DeserializeObject<CTrademarkRegistrationInfo>(m_json);
