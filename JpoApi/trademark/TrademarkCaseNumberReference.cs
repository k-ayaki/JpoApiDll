    ...
    public class TrademarkCaseNumberReference : IDisposable
    {
        ...
        public TrademarkCaseNumberReference(string szType, string szNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_json = jsonCache.GetJson("api/trademark/v1/case_number_reference/" + szType + "/" + szNumber);
                    this.m_jsonFile = jsonCache.m_jsonFilePath;
                    this.m_error = jsonCache.m_error;

                    if (m_json.Length > 0)
                    {
                        CTrademarkCaseNumberReference jsonObj = JsonConvert.DeserializeObject<CTrademarkCaseNumberReference>(m_json);
