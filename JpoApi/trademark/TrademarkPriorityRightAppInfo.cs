    ...
    public class TrademarkPriorityRightAppInfo : IDisposable
    {
        ...
        public TrademarkPriorityRightAppInfo(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/trademark/v1/priority_right_app_info/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CTrademarkPriorityRightAppInfo jsonObj = JsonConvert.DeserializeObject<CTrademarkPriorityRightAppInfo>(m_response);
