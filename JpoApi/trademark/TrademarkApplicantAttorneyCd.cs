    ...
    public class TrademarkApplicantAttorneyCd : IDisposable
    {
        ...
        public TrademarkApplicantAttorneyCd(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_json = jsonCache.GetJson("api/trademark/v1/applicant_attorney_cd/" + applicationNumber);
                    this.m_jsonFile = jsonCache.m_jsonFilePath;
                    this.m_error = jsonCache.m_error;

                    if (m_json.Length > 0)
                    {
                        CDesignApplicantAttorneyCd jsonObj = JsonConvert.DeserializeObject<CDesignApplicantAttorneyCd>(m_json);
