...
namespace JpoApi.trademark
{
    public class TrademarkJppFixedAddress : IDisposable
    {
        ...
        private class CTrademarkJppFixedAddress
        {
        ...
        public TrademarkJppFixedAddress(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                this.m_response = responseCache.GetJson("api/trademark/v1/jpp_fixed_address/" + applicationNumber);
                this.m_responseFile = responseCache.m_responseFilePath;
                this.m_error = responseCache.m_error;

                if (m_response.Length > 0)
                {
                    CTrademarkJppFixedAddress jsonObj = JsonConvert.DeserializeObject<CTrademarkJppFixedAddress>(m_response);
