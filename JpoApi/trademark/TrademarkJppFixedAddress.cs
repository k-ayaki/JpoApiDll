    ...
    public class TrademarkJppFixedAddress : IDisposable
    {
        ...
        public TrademarkJppFixedAddress(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                this.m_json = jsonCache.GetJson("api/trademark/v1/jpp_fixed_address/" + applicationNumber);
                this.m_jsonFile = jsonCache.m_jsonFilePath;
                this.m_error = jsonCache.m_error;

                if (m_json.Length > 0)
                {
                    CTrademarkJppFixedAddress jsonObj = JsonConvert.DeserializeObject<CTrademarkJppFixedAddress>(m_json);
