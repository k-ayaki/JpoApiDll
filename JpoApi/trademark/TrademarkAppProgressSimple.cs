    ...
    public class TrademarkAppProgressSimple : IDisposable
    {
        ...
        public TrademarkAppProgressSimple(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_json = jsonCache.GetJson("api/trademark/v1/app_progress_simple/" + applicationNumber);
                    this.m_jsonFile = jsonCache.m_jsonFilePath;
                    this.m_error = jsonCache.m_error;

                    if (m_json.Length > 0)
                    {
                        CTrademarkAppProgressSimple jsonObj = JsonConvert.DeserializeObject<CTrademarkAppProgressSimple>(m_json);
