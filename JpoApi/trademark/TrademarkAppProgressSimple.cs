...
namespace JpoApi
{
    public class TrademarkAppProgressSimple : IDisposable
    {
        ...
        public class CData             // 詳細情報データ
        {
            ...
            public Dictionary<string, string> transliteration { get; set; }
            public Dictionary<string, string> viennaClass { get; set; }
        ...
        private class CTrademarkAppProgressSimple
        {
        ...
        public TrademarkAppProgressSimple(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/trademark/v1/app_progress_simple/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CTrademarkAppProgressSimple jsonObj = JsonConvert.DeserializeObject<CTrademarkAppProgressSimple>(m_response);
