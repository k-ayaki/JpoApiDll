    ...
    public class PctNationalPhaseApplicationNumber : IDisposable
    {
        ...
        public PctNationalPhaseApplicationNumber(string szKind, string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/patent/v1/pct_national_phase_application_number/" + szKind + "/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CPctNationalPhaseApplicationNumber jsonObj = JsonConvert.DeserializeObject<CPctNationalPhaseApplicationNumber>(m_response);
