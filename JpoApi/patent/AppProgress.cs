...
namespace JpoApi
{
    ...
    public class AppProgress : IDisposable  
    {
        ...
        public AppProgress(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/patent/v1/app_progress/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CAppProgress jsonObj = JsonConvert.DeserializeObject<CAppProgress>(m_response);
