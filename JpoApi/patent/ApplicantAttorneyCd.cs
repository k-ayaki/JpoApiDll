    ...
    public class ApplicantAttorneyCd : IDisposable
    {
        ...
        public ApplicantAttorneyCd(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/patent/v1/applicant_attorney_cd/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CApplicantAttorneyCd jsonObj = JsonConvert.DeserializeObject<CApplicantAttorneyCd>(m_response);
