    ...
    public class CiteDocInfo : IDisposable
    {
        ...
        public CiteDocInfo(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/patent/v1/cite_doc_info/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CCitedDocInfo jsonObj = JsonConvert.DeserializeObject<CCitedDocInfo>(m_response);
