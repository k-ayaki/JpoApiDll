    ...
    public class DivisionalAppInfo : IDisposable
    {
        ...
        public DivisionalAppInfo(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/patent/v1/divisional_app_info/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CDivisionalAppInfo jsonObj = JsonConvert.DeserializeObject<CDivisionalAppInfo>(m_response);
