    ...
    public class JppFixedAddress : IDisposable
    {
        ...
        public JppFixedAddress(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                this.m_response = responseCache.GetJson("api/patent/v1/jpp_fixed_address/" + applicationNumber);
                this.m_responseFile = responseCache.m_responseFilePath;
                this.m_error = responseCache.m_error;

                if (m_response.Length > 0)
                {
                    CJppFixedAddress jsonObj = JsonConvert.DeserializeObject<CJppFixedAddress>(m_response);
