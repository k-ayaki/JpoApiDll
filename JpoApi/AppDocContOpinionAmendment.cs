...
namespace JpoApi
{
    ...
    public class AppDocContOpinionAmendment : IDisposable
    {
        ...
        public string m_remainAccessCount { get; set; }
        ...
        private class CAppDocContOpinionAmendment
        {
            public CResult result { get; set; }
        }
        public AppDocContOpinionAmendment(string requestNumber, string a_access_token, string cacheDir = null)
        {
            try
            {
                m_error = e_NONE;
                ...
                {
                    m_cacheDir = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\ayaki\jpoapi\app_doc_cont_opinion_amendment";
        ...
        public AppDocContOpinionAmendment(string cacheDir = null)
        {
            try
            {
                m_error = e_NONE;
                ...
                if (File.Exists(m_jsonFile))
                {
                    ...
                    if (dt.AddDays(-ac.m_cacheEffective).Date <= fi.LastWriteTime.Date)
                    {
                        using (JpoHttp jpo = new JpoHttp())
                        {
                            ...
                            CAppDocContOpinionAmendment applicant = JsonConvert.DeserializeObject<CAppDocContOpinionAmendment>(jpo.m_json);
                            ...
                            switch (m_cache_result.statusCode)
                            {
                                ...
                                default:
                                    // jsonファイルを削除
                                    if (System.IO.File.Exists(m_jsonFile))
                                        File.Delete(m_jsonFile);
                                    break;
                            }
                            jpo.Dispose();
                            return m_error;
        ...
        private void read(string requestNumber, string a_access_token)
        {
            using (JpoHttp jpo = new JpoHttp())
            {
                ...
                jpo.get(Properties.Settings.Default.url + @"/app_doc_cont_opinion_amendment/" + requestNumber, a_access_token);
                while (DateTime.Now.Second % 6 == networkState.m_i64macaddress % 6)
                {
                    System.Threading.Thread.Sleep(100);
                }
