    ...
    public class DesignRegistrationInfo : IDisposable
    {
        ...
        public DesignRegistrationInfo(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/design/v1/registration_info/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CDesignRegistrationInfo jsonObj = JsonConvert.DeserializeObject<CDesignRegistrationInfo>(m_response);
