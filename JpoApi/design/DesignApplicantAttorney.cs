    ...
    public class DesignApplicantAttorney : IDisposable
    {
        ...
        public DesignApplicantAttorney(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/design/v1/applicant_attorney/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CDesignApplicantAttorney jsonObj = JsonConvert.DeserializeObject<CDesignApplicantAttorney>(m_response);
