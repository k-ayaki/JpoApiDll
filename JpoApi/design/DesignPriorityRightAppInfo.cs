...
{
    // 特許優先基礎出願情報
    public class DesignPriorityRightAppInfo : IDisposable
    {
        ...
        private class CDesignPriorityRightAppInfo
        {
        ...
        public DesignPriorityRightAppInfo(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/design/v1/priority_right_app_info/" + applicationNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CDesignPriorityRightAppInfo jsonObj = JsonConvert.DeserializeObject<CDesignPriorityRightAppInfo>(m_response);
