    ...
    public class DesignPriorityRightAppInfo : IDisposable
    {
        ...
        public DesignPriorityRightAppInfo(string applicationNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_json = jsonCache.GetJson("api/design/v1/priority_right_app_info/" + applicationNumber);
                    this.m_jsonFile = jsonCache.m_jsonFilePath;
                    this.m_error = jsonCache.m_error;

                    if (m_json.Length > 0)
                    {
                        CDesignPriorityRightAppInfo jsonObj = JsonConvert.DeserializeObject<CDesignPriorityRightAppInfo>(m_json);
