...
{
    // 意匠番号参照
    public class DesignCaseNumberReference : IDisposable
    {
        ...
        public class CData
        {
            ...
            public string internationalRegistrationNumber { get; set; }  // 国際登録番号
            public string designNumber { get; set; }  // 意匠番号
        }
        ...
        private class CDesignCaseNumberReference
        {
        ...
        public DesignCaseNumberReference(string szType, string szNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_response = responseCache.GetJson("api/design/v1/case_number_reference/" + szType + "/" + szNumber);
                    this.m_responseFile = responseCache.m_responseFilePath;
                    this.m_error = responseCache.m_error;

                    if (m_response.Length > 0)
                    {
                        CDesignCaseNumberReference jsonObj = JsonConvert.DeserializeObject<CDesignCaseNumberReference>(m_response);
