    ...
    public class DesignCaseNumberReference : IDisposable
    {
        ...
        public DesignCaseNumberReference(string szType, string szNumber, string a_access_token = "")
        {
            try
            {
                ...
                {
                    this.m_json = jsonCache.GetJson("api/design/v1/case_number_reference/" + szType + "/" + szNumber);
                    this.m_jsonFile = jsonCache.m_jsonFilePath;
                    this.m_error = jsonCache.m_error;

                    if (m_json.Length > 0)
                    {
                        CDesignCaseNumberReference jsonObj = JsonConvert.DeserializeObject<CDesignCaseNumberReference>(m_json);
